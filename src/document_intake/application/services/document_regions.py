...
from document_intake.application.services import document_region_media as _media
from document_intake.application.services import document_region_persistence as _persistence
from document_intake.application.services import document_region_validation as _validation
...
class _ReadContext:
    ...
    recipe_snapshots: tuple[_persistence.RecipeReadSnapshot, ...]


def _validate_command(command: ConfirmDocumentRegionsCommand) -> None:
    _validation.validate_source_independent_command(command)
    _validation.validate_region_count(command)
    _validation.validate_contiguous_order_indices(command)
    created = _validation.validate_created_record_id_distinctness(command)
    _validation.validate_exactly_one_selection_form(command)
    _validation.validate_new_revision_region_identity(command, created)
    _validation.reject_command_level_duplicates(command)
...
def _load_preceding_set(
    ...
    if command.superseded_region_set_version_id is None:
        _persistence.fail(DocumentRegionErrorCode.REGION_SET_NOT_FOUND)
...
) -> tuple[tuple[ImageGeometryRecipe, ...], tuple[_persistence.RecipeReadSnapshot, ...]]:
    ...
    for member in command.members:
        ...
        if isinstance(selection, ExistingRecipeSelection):
            ...
            if not (valid_root or valid_later):
                _persistence.fail(DocumentRegionErrorCode.REGION_REVISION_CONFLICT)
            recipe = _new_recipe(command, source, member, selection)
            exact = latest_at_read = latest
        selected.append(recipe)
        snapshots.append(_persistence.RecipeReadSnapshot(recipe, exact, latest_at_read))
...
def _validate_complete_selected_set(selected: tuple[ImageGeometryRecipe, ...]) -> None:
    ...
    if len(recipe_ids) != len(selected) or len(quadrilaterals) != len(selected):
        _persistence.fail(DocumentRegionErrorCode.DUPLICATE_REGION)
...
def _verify_region_revisions(
    command: ConfirmDocumentRegionsCommand, read: _ReadContext, readback: _persistence.WriteReadback
) -> None:
...
def _reread_and_revalidate_selected_state(
    ...
    exact_recipes, latest_recipes = _persistence.reread_recipe_state(command, uow)
    if previous != read.previous_set or any(
        ...
        _persistence.fail(DocumentRegionErrorCode.PERSISTED_DATA_INVALID)
    return _persistence.WriteReadback(previous, latest_set, exact_recipes, latest_recipes)
...
def _verify_set_revision(
    ...
    if (
        ...
        _persistence.fail(DocumentRegionErrorCode.REGION_SET_REVISION_CONFLICT)
    if command.set_revision > 1:
...
def confirm_document_regions(
    ...
    *,
    ...
    try:
        ...
        _persistence.map_controlled_failure(error)
    except Exception:
        _persistence.fail(DocumentRegionErrorCode.PERSISTENCE_FAILED)
    ...
    try:
        ...
        with write_cm as write_uow:
            ...
            region_set = _persistence.persist_confirmation(command, context.selected, write_uow)
            try:
                write_uow.commit()
            except Exception:
                _persistence.fail(DocumentRegionErrorCode.COMMIT_FAILED)
    except DocumentRegionsError:
        raise
    except PersistenceError as error:
        _persistence.map_controlled_failure(error, late=True)
    except Exception:
        _persistence.fail(DocumentRegionErrorCode.PERSISTENCE_FAILED)
