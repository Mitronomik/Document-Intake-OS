...
_VERSION_CREATE = """CREATE TABLE document_side_composition_versions(
id TEXT PRIMARY KEY NOT NULL CHECK(length(id)=36),
composition_id TEXT NOT NULL UNIQUE REFERENCES document_side_compositions(id),
side_1_region_set_version_id TEXT NOT NULL REFERENCES document_region_set_versions(region_set_version_id),
side_1_source_file_id TEXT NOT NULL REFERENCES source_files(id),
side_1_region_id TEXT NOT NULL CHECK(length(side_1_region_id)=36),
side_1_geometry_recipe_version_id TEXT NOT NULL REFERENCES image_geometry_recipes(recipe_version_id),
side_2_region_set_version_id TEXT NOT NULL REFERENCES document_region_set_versions(region_set_version_id),
side_2_source_file_id TEXT NOT NULL REFERENCES source_files(id),
side_2_region_id TEXT NOT NULL CHECK(length(side_2_region_id)=36),
side_2_geometry_recipe_version_id TEXT NOT NULL REFERENCES image_geometry_recipes(recipe_version_id),
layout TEXT NOT NULL CHECK(layout IN ('VERTICAL','HORIZONTAL')),
outer_margin_px INTEGER NOT NULL CHECK(outer_margin_px BETWEEN 0 AND 256),
inter_side_gap_px INTEGER NOT NULL CHECK(inter_side_gap_px BETWEEN 0 AND 256),
composition_pipeline_id TEXT NOT NULL CHECK(composition_pipeline_id='PILLOW_DOCUMENT_SIDE_COMPOSITION_BICUBIC'),
composition_pipeline_version INTEGER NOT NULL CHECK(composition_pipeline_version=1),
jpeg_pipeline_id TEXT NOT NULL CHECK(jpeg_pipeline_id='PILLOW_PREPARED_JPEG'),
jpeg_pipeline_version INTEGER NOT NULL CHECK(jpeg_pipeline_version=1),
output_contract_id TEXT NOT NULL CHECK(output_contract_id='PREPARED_JPEG_SRGB_V1'),
output_contract_version INTEGER NOT NULL CHECK(output_contract_version=1),
created_at_utc TEXT NOT NULL CHECK(length(created_at_utc)>=20 AND substr(created_at_utc,-1)='Z'),
created_by_id TEXT NOT NULL CHECK(length(created_by_id)=36),
created_by_kind TEXT NOT NULL CHECK(created_by_kind IN ('OPERATOR','ADMIN','SYSTEM')),
correlation_id TEXT NOT NULL CHECK(length(correlation_id)=36),
canonical_payload TEXT NOT NULL CHECK(length(canonical_payload)>0),
CHECK(side_1_source_file_id<>side_2_source_file_id OR side_1_region_id<>side_2_region_id),
...
REFERENCES document_region_set_members(region_set_version_id,geometry_recipe_version_id),
UNIQUE(side_1_region_set_version_id,side_1_source_file_id,side_1_region_id,side_1_geometry_recipe_version_id,side_2_region_set_version_id,side_2_source_file_id,side_2_region_id,side_2_geometry_recipe_version_id,layout,outer_margin_px,inter_side_gap_px,composition_pipeline_id,composition_pipeline_version,jpeg_pipeline_id,jpeg_pipeline_version,output_contract_id,output_contract_version)
)"""  # noqa: E501

_SIDE_1_LINEAGE_GUARD = """CREATE TRIGGER document_side_composition_versions_side_1_lineage_guard
BEFORE INSERT ON document_side_composition_versions
WHEN NOT EXISTS(
SELECT 1
FROM document_region_set_versions AS region_sets
JOIN document_region_set_members AS members
ON members.region_set_version_id=region_sets.region_set_version_id
JOIN image_geometry_recipes AS recipes
ON recipes.recipe_version_id=members.geometry_recipe_version_id
WHERE region_sets.region_set_version_id=NEW.side_1_region_set_version_id
AND region_sets.source_file_id=NEW.side_1_source_file_id
AND members.region_set_version_id=NEW.side_1_region_set_version_id
AND members.region_id=NEW.side_1_region_id
AND members.geometry_recipe_version_id=NEW.side_1_geometry_recipe_version_id
AND recipes.recipe_version_id=NEW.side_1_geometry_recipe_version_id
AND recipes.source_file_id=NEW.side_1_source_file_id
AND recipes.region_id=NEW.side_1_region_id
)
BEGIN SELECT RAISE(ABORT,'document side lineage invalid'); END"""

_SIDE_2_LINEAGE_GUARD = _SIDE_1_LINEAGE_GUARD.replace("side_1", "side_2").replace(
    "versions_side_1_lineage_guard", "versions_side_2_lineage_guard"
)
