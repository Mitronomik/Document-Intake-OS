...
def _transform_geometry_payloads(connection: Any) -> None:
    ...
    for row in rows:
        ...
        _verify_inserted_recipe(connection, recipe)
        inserted += 1
...
def _verify_inserted_recipe(connection: Any, expected: Any) -> None:
    row = connection.execute(
        "SELECT recipe_version_id,source_file_id,region_id,superseded_recipe_version_id,revision,coordinate_space,source_effective_width,source_effective_height,quarter_turn_clockwise,top_left_x,top_left_y,top_right_x,top_right_y,bottom_right_x,bottom_right_y,bottom_left_x,bottom_left_y,geometry_pipeline_id,geometry_pipeline_version,created_at_utc,canonical_payload FROM image_geometry_recipes_v0008_new WHERE recipe_version_id=?",  # noqa: E501
        (str(expected.recipe_version_id),),
    ).fetchone()
    if row is None:
        raise PersistenceError(PersistenceErrorCode.PERSISTED_DATA_INVALID)
    restored = geometry_ser.image_geometry_recipe_from_json(row[20])
    if (
        restored != expected
        or geometry_ser.image_geometry_recipe_columns(restored) != tuple(row[:20])
        or geometry_ser.image_geometry_recipe_to_json(restored) != row[20]
    ):
        raise PersistenceError(PersistenceErrorCode.PERSISTED_DATA_INVALID)
...
STATEMENTS = (
    ...
    "CREATE INDEX image_geometry_recipes_source_region_order_idx ON image_geometry_recipes(source_file_id,region_id,revision,created_at_utc,recipe_version_id)",  # noqa: E501
    "CREATE TRIGGER image_geometry_recipes_no_update BEFORE UPDATE ON image_geometry_recipes BEGIN SELECT RAISE(ABORT,'image_geometry_recipes_append_only'); END",  # noqa: E501
    "CREATE TRIGGER image_geometry_recipes_no_delete BEFORE DELETE ON image_geometry_recipes BEGIN SELECT RAISE(ABORT,'image_geometry_recipes_append_only'); END",  # noqa: E501
    "CREATE TRIGGER image_geometry_recipes_no_replace BEFORE INSERT ON image_geometry_recipes WHEN EXISTS(SELECT 1 FROM image_geometry_recipes WHERE recipe_version_id=NEW.recipe_version_id) BEGIN SELECT RAISE(ABORT,'image_geometry_recipes_duplicate'); END",  # noqa: E501
    "CREATE TABLE document_region_set_versions(region_set_version_id TEXT PRIMARY KEY NOT NULL CHECK(length(region_set_version_id)=36),source_file_id TEXT NOT NULL REFERENCES source_files(id),superseded_region_set_version_id TEXT NULL REFERENCES document_region_set_versions(region_set_version_id),revision INTEGER NOT NULL CHECK(revision>=1),confirmed_at_utc TEXT NOT NULL,confirmed_by_actor_id TEXT NOT NULL CHECK(length(confirmed_by_actor_id)=36),confirmed_by_actor_kind TEXT NOT NULL CHECK(confirmed_by_actor_kind IN ('OPERATOR','ADMIN','SYSTEM')),canonical_payload TEXT NOT NULL,UNIQUE(source_file_id,revision),UNIQUE(superseded_region_set_version_id))",  # noqa: E501
    "CREATE TABLE document_region_set_members(region_set_version_id TEXT NOT NULL REFERENCES document_region_set_versions(region_set_version_id),order_index INTEGER NOT NULL CHECK(order_index IN (1,2)),region_id TEXT NOT NULL CHECK(length(region_id)=36),geometry_recipe_version_id TEXT NOT NULL REFERENCES image_geometry_recipes(recipe_version_id),PRIMARY KEY(region_set_version_id,order_index),UNIQUE(region_set_version_id,region_id),UNIQUE(region_set_version_id,geometry_recipe_version_id))",  # noqa: E501
    "CREATE INDEX document_region_sets_source_order_idx ON document_region_set_versions(source_file_id,revision)",  # noqa: E501
    "CREATE TRIGGER document_region_set_versions_no_update BEFORE UPDATE ON document_region_set_versions BEGIN SELECT RAISE(ABORT,'document_region_set_versions immutable'); END",  # noqa: E501
    "CREATE TRIGGER document_region_set_versions_no_delete BEFORE DELETE ON document_region_set_versions BEGIN SELECT RAISE(ABORT,'document_region_set_versions immutable'); END",  # noqa: E501
    "CREATE TRIGGER document_region_set_versions_no_replace BEFORE INSERT ON document_region_set_versions WHEN EXISTS(SELECT 1 FROM document_region_set_versions WHERE region_set_version_id=NEW.region_set_version_id) BEGIN SELECT RAISE(ABORT,'document_region_set_versions duplicate'); END",  # noqa: E501
    "CREATE TRIGGER document_region_set_members_no_update BEFORE UPDATE ON document_region_set_members BEGIN SELECT RAISE(ABORT,'document_region_set_members immutable'); END",  # noqa: E501
    "CREATE TRIGGER document_region_set_members_no_delete BEFORE DELETE ON document_region_set_members BEGIN SELECT RAISE(ABORT,'document_region_set_members immutable'); END",  # noqa: E501
    "CREATE TRIGGER document_region_set_members_no_replace BEFORE INSERT ON document_region_set_members WHEN EXISTS(SELECT 1 FROM document_region_set_members WHERE region_set_version_id=NEW.region_set_version_id AND order_index=NEW.order_index) BEGIN SELECT RAISE(ABORT,'document_region_set_members duplicate'); END",  # noqa: E501
)
MIGRATION = Migration(
    8,
