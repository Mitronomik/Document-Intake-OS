...
STATEMENTS = (
    ...
    CREATE TABLE audit_events(
        ...
        field_key TEXT NULL CHECK(field_key IS NULL OR (length(field_key) BETWEEN 1 AND 128 AND field_key GLOB '[a-z0-9]*')),
        ...
        before_display_value TEXT NULL CHECK(before_display_value IS NULL OR (length(before_display_value) BETWEEN 1 AND 64 AND before_display_value GLOB '[A-Z0-9]*')),
        ...
        after_display_value TEXT NULL CHECK(after_display_value IS NULL OR (length(after_display_value) BETWEEN 1 AND 64 AND after_display_value GLOB '[A-Z0-9]*')),
        reason_code TEXT NULL CHECK(reason_code IS NULL OR (length(reason_code) BETWEEN 1 AND 64 AND reason_code GLOB '[A-Z]*')),
