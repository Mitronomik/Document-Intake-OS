...
## PR-010 geometry recipe domain staging

ADR-024 proposes immutable append-only geometry recipe versions using `SOURCE_EFFECTIVE_PIXELS_V1`, canonical `top_left`, `top_right`, `bottom_right`, `bottom_left` quadrilateral order, coarse clockwise quarter-turn rotation, and one recipe for one manually selected document area from one source file. Multiple document regions remain PR-012.
