...
9. `docs/progress.md` and `docs/handoff.md` agree that PR-002 is the current repository-safety task, formal M1 entry is not asserted while M0 remains open, and PR-003 is blocked until PR-002 is accepted and an explicit M0/M1 sequencing decision is recorded and permits continuation.
...
Formal M1 entry is therefore not asserted in PR-002. The sequencing of M0 and the PR-001–PR-003 repository-safety work requires an explicit product-owner decision. PR-003 must not begin before PR-002 acceptance and an explicit M0/M1 sequencing decision that permits continuation. PR-002 reports the conflict but does not add an ADR or make that product-owner decision.
